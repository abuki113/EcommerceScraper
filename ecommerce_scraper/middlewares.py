...
class CpuscraperSpiderMiddleware:
    # Not all methods need to be defined. If a method is not defined,
    # scrapy acts as if the spider middleware does not modify the
    # passed objects.
...
class CpuscraperDownloaderMiddleware:
    # Not all methods need to be defined. If a method is not defined,
    # scrapy acts as if the downloader middleware does not modify the
    # passed objects.
...
class ScrollToBottomMiddleware:
    def __init__(self):
        self.loop = asyncio.get_event_loop()

    async def scroll_to_bottom(self, page):
        # Scroll to the bottom of the page using Playwright
        await page.evaluate('window.scrollTo(0, document.body.scrollHeight)')

    async def process_response(self, request, response, spider):
        # Only apply scrolling to specific responses if needed
        if 'your_target_url' in response.url:
            async with async_playwright() as p:
                browser = await p.chromium.launch()
                page = await browser.new_page()
                await page.goto(response.url)

                # Simulate scrolling until the bottom
                while True:
                    old_position = await page.evaluate('window.scrollY')
                ...
                return response.replace(body=final_html.encode('utf-8'))
